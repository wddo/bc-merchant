...
Swiper.use([Navigation, Pagination, Autoplay, Mousewheel, FreeMode, Scrollbar]);
...
class CustomSwiper {
  ...
  constructor(target) {
    const el = {
      ...
      btnStop: target.querySelector(
        '.btn--stop, .swiper-button-autoplay--pause'
      ),
      btnPlay: target.querySelector(
        '.btn--Play, .swiper-button-autoplay--play'
      ),
    ...
    if (optionData.slidesPerGroup) {
      options.slidesPerGroup = optionData.slidesPerGroup;
    }
    ...
    if (optionData.autoplay > 0) {
      if (!el.swiperWrapper.querySelector('.swiper-autoplay')) {
        el.btnStop = el.swiperWrapper.querySelector(
          '.btn--stop, .swiper-button-autoplay--pause'
        );
        el.btnPlay = el.swiperWrapper.querySelector(
          '.btn--Play, .swiper-button-autoplay--play'
        );
      }
    ...
    if (optionData.pagination === 'all') {
      ...
      options.pagination = {
        ...
        renderBullet: function(i) {
          return `<button class="swiper-pagination-bullet swiper-pagination-bullet-active" type="button" aria-label="Go to slide ${
            i + 1
          }"></button>`;
          //'<button type="button" class="swiperMore">더보기</button></div>'
        },
    ...
    options.on = {
      ...
      observerUpdate: () => {
        optionData.loop === true
          ? swiper.slideToLoop(rIndex, 0)
      ...
      slideChangeTransitionStart: function() {
        if (el.progressBar) {
          const total =
            this.slides.length -
            this.el.querySelectorAll('.swiper-slide-duplicate').length;
          if (
            options.loop === false &&
            Object.keys(options.breakpoints).length > 0
          ) {
            //breakpoints 값 있을 경우
            el.progressBar.style.transform = `scale(${
              (this.snapIndex + 1) / this.snapGrid.length
            } ,1)`;
          } else {
            el.progressBar.style.transform = `scale(${
        ...
        // rIndex = this.realIndex ? this.realIndex : this.activeIndex;
        ...
        if (el.swiperWrapper.querySelector('.swiper-slide-active')) {
          const slide = el.swiperWrapper.querySelector('.swiper-slide-active');
          if (slide) {
            slide.classList.add('main-swiper-slide-active');
          }
        }
      ...
      init() {
        ...
        if (options.slidesPerView !== 'auto') {
          ...
          el.swiperWrapper.querySelector('.swiper-slide-active')
            ? el.swiperWrapper
      ...
      slideChangeTransitionEnd() {
        ...
        if (options.pagination && options.pagination.type === 'custom') {
          let btnBullet = el.swiperWrapper.querySelectorAll(
    ...
    const handler = {
      ...
      resizeSlide: () => {
        if (
          utils.isPc() &&
          el.container.getAttribute('data-wrapSlideCnt') === '3'
        ) {
          //PC 슬라이드 설정
          ...
          !utils.isPc() &&
          el.container.getAttribute('data-wrapSlideCnt') === '5'
        ) {
          //MOB 슬라이드 설정
      ...
      keydownBtnBullet: (e) => {
        if (e.keyCode === 13) {
          let index = Array.from(
            e.currentTarget.parentNode.querySelectorAll(
              '.swiper-pagination-bullet'
            )
    ...
    const method = {
      wrapSlide: () => {
        ...
        let nodesToWrap = document.querySelectorAll(
          '.thumbnail-carousel-box__swiper-slide'
        );
        let node, wrapper;
      ...
      initSwiper: () => {
        ...
        if (el.swiperSlide.length > 1) {
          ...
          swiper = new Swiper(el.container, options);
          swiper.on('slidesLengthChange', () => {
            console.log('=== slidesLengthChange ===');
            swiperDestroy();
          });
    ...
    const bind = () => {
      ...
      //모드 변경 시 슬라이드 묶음 변경
      if (
        el.container.classList.contains('thumbnail-carousel-box__swiper-wrap')
      ) {
      ...
      if (options.autoplay !== false && options.autoplay !== '0') {
        ...
        el.swiperWrapper.addEventListener(
          'mouseleave',
          handler.mouseleaveSwiper
        );
    ...
    const unbind = () => {
      ...
      if (options.autoplay !== false) {
        el.swiperWrapper.removeEventListener(
          'mouseover',
          handler.mouseoverSwiper
        );
        el.swiperWrapper.removeEventListener(
          'mouseleave',
          handler.mouseleaveSwiper
...
mvJs.customSwiper = {
  ...
  setPlay: (id) => {
    ...
    swiper.params.autoplay.delay = parseInt(optionData.autoplay, 10);
