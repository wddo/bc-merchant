...
const pbut = {
	...
	getFocusableElements: function(container) {
		...
		if ($container.length === 0) return { elements: $(), first: null, last: null };
...
const pbui = (function() {
	...
	class Popup {
		constructor(target, obj={}) {
			...
			const handler = {
				...
				resize: (e) => {
					if (isTooltipType) el.$popup.is(':visible') && el.$popup.css(method.getPosition());
				}
			};
			...
			const method = {
				open: () => {
					...
					el.$opener = option.opener ? $(option.opener) : $(document.activeElement);
					el.$popup.trigger('openbefore', el);

					this.state = Popup.STATE_OPEN;

					if (isTooltipType) el.$popup.css({left:0,top:$(window).scrollTop + 'px'});
				...
				getPosition: () => {
					...
					switch (option.position) {
						case 'top-center':
							...
							left = realCoord.x - el.$popup.outerWidth()/2 + el.$opener.outerWidth()/2 
							break;
						case 'top-right':
							top = realCoord.y - el.$popup.outerHeight() - margin
							left = realCoord.x  - el.$popup.outerWidth() + el.$opener.outerWidth()
							break;
						case 'middle-left':
							top = realCoord.y - el.$popup.outerHeight()/2 + el.$opener.outerWidth()/2
		...
		constructor(obj = {}) {
			...
			const method = {
				render: () => {
					...
					const $outer = $('<div class="_modal-alert" aria-hidden="true"></div>');
					...
					if (option.buttonRender && option.buttonRender.length) {
						...
						const $defaultBtn = $('<button type="button" class="_btn-medium-primary">확인</button>');
						$defaultBtn.on('click.alert', method.close);
						$footer.append($defaultBtn);
					}

					if (option.closeButton) {
						const closeBtn = $('<button class="btn-close" type="button">닫기</button>')
		...
		constructor(target, obj = {}) {
			...
			const handler = {
				...
				outerClick: (e) => {
					...
					if (el.$tooltip) {
						if (!$target.closest(el.$opener[0]).length && !$target.closest(el.$tooltip[0]).length) {
							method.close();
						}
					}
				},
			...
			const method = {
				render: () => {
					...
					el.$tooltip.find('.tooltip-content').html(option.content);
		...
		constructor(target, obj = {}) {
			...
			let option = {
				...
				create: function (event, ui) {
					...
					if (!$selectedCont.attr('selected') || $selectedCont.attr('disabled')) {
				...
				open: function () {
					...
					if (!$(this).selectmenu('widget').find('.ui-selectmenu-text').hasClass('placeholder')) {
			...
			const handler = {
				scroll: () => {
					if (pbut.getDeviceType() === 'mobile') { } else {
						$('.ui-selectmenu-menu').filter(':visible').each(function () {
							const $this = $(this);
							const $select = $(`#${$this.children('ul').attr('aria-labelledby')}`).prev('select');
						});
					}
				},
				resize: () => {
					...
					$select.each(function () {
						...
						if (pbut.getDeviceType() === 'mobile' || $selectWrap.is(':hidden')) {
							$menuWrap.width('');
						} else {
					...
					if (pbut.getDeviceType() === 'mobile') {
						...
						menuWidget.off('touchstart').on('touchstart', (e) => {
							...
							menuWidget.on('touchend', (e) => {
								...
								if ((startY < endY && endTime - startTime < 150) || (menuH / 2 > menuH - parseInt(menuWidget.css('top')))) {
									if (startY !== endY) {
	...
	const popupCtl = {
		init: function(selector, obj = {}) {
			...
			if (popup) {
				...
				_weakMap.set(el, new Popup(el, obj));
			}
		},
	...
	const tooltipCtl = {
		init: function(selector, obj = {}) {
			...
			let tooltip = _weakMap.get(el);

			if (!tooltip) {
				tooltip = new Tooltip(el, obj);
	...
	const fn_iptSetFocus = function () {
		$(document).on('focusin', 'input[type=text]:not([readonly]), input[type=password]:not([readonly])', function () {
			...
			function positionButton() {
				const right = $parent.innerWidth() - ($input.position().left + $input.outerWidth()) + 12;
				$clearBtn.css('right', right + 'px');
			}
			positionButton();

			$(window).off('resize.iptSetFocus').on('resize.iptSetFocus', positionButton);

			// 버튼 클릭 시 focusout 방지
			$clearBtn.off('mousedown.iptSetFocus').on('mousedown.iptSetFocus', function (e) {
				e.preventDefault();
			});

			// 버튼 클릭
			$clearBtn.off('click.iptSetFocus').on('click.iptSetFocus', function (e) {
				e.preventDefault();
				$input.val('');
				$clearBtn.hide();
				$input.focus();
	...
	const fn_passwordMask = function () {
		...
		$maskBtns.each(function () {
			...
			// 클릭 이벤트
			$maskBtn
				.off('click.passwordMask')
				.on('click.passwordMask', function (e) {
					e.preventDefault();
					toggle($maskBtn);
				});
